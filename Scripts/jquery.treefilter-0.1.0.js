    ...
    var searcher = function () {
        $(plugin.settings.searcher).keyup(function () {
            if ($(this).val().length == 0) {
                ...
                //plugin.el.find("li:containsIN('" + $(this).val() + "')").addClass("high");
                //alert($(this).val());
                plugin.el.find("li.tf-search-result").parent().addClass("tf-search-result");

                //"li:containsIN('" + $(this).val() + "')"
//                $("div", "#mytree").removeClass("high");
//                $("div:containsIN('" + $(this).val() + "')", "#mytree").addClass("high");
                ////                if ("div:containsIN("+$(this).val()+")") {
                //                    plugin.el.find("div:containsIN('" + $(this).val() + "')").addClass("high");
                //                } else {
                //                    plugin.el.find("div:containsIN('" + $(this).val() + "')").removeClass("high");
                //                }
                //plugin.el.find("div:containsIN('" + $(this).val() + "')").addClass("high");
            }
        });
